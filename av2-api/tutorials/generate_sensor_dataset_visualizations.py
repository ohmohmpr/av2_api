...
def generate_sensor_dataset_visualizations(
    ...
    for _, datum in enumerate(track(dataset, "Creating sensor tutorial videos ...")):
        ...
        print("datum.sweep_number", datum.sweep_number)
        print("datum.num_sweeps_in_log - 1", datum.num_sweeps_in_log - 1)
        
        if datum.sweep_number == datum.num_sweeps_in_log - 1 or (datum.sweep_number % 40 == 0 and \
        datum.sweep_number != 0):
            video: NDArrayByte = np.stack(tiled_cams_list)
            dst_path = Path("videos") / f"{datum.log_id}_{c}.mp4"
